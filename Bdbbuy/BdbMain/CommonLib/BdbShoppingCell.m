...
@property (nonatomic, strong) BdbBuyView *buyView;
@end
@implementation BdbShoppingCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _buyView = [[BdbBuyView alloc]init];
...
- (void)shoppingSelectedClick:(UIButton *)btn{
    if (btn.selected) {
        btn.selected = NO;
        [[BdbUserShopCarTool sharedInstance]removeFromProductShopCar:self.goods];
        [[NSNotificationCenter defaultCenter] postNotificationName:LFBShopCarBuyNumberDidChangeNotification object:nil];
    }else{
        btn.selected = YES;;
        [[BdbUserShopCarTool sharedInstance]addSupermarkProductToShopCar:self.goods];
        [[NSNotificationCenter defaultCenter] postNotificationName:LFBShopCarBuyNumberDidChangeNotification object:nil];
    }
...
- (void)setGoods:(BdbGoods *)goods{
    ...
    _moneyLabel.text = [NSString stringWithFormat:@"CA$ %@",goods.price];
