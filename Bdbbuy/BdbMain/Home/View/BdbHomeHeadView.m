...
@implementation BdbHomeHeadView
- (instancetype)initWithHeadData:(BdbHomeHeadData *)headData{
    ...
    _scrollerView = [BdbScrollerPageView pageScroller:focusImages placeHolderImage:[UIImage imageNamed:@"v2_placeholder_full_size"]];
    _scrollerView.backgroundColor = [UIColor orangeColor];
    _hotView = [[BdbHotView alloc]initWithImages:iconImages title:iconTitles placeHolder:[UIImage imageNamed:@"v2_placeholder_square"] ];
    _headlineView = [[BdbHeadLineView alloc]init];
...
- (void)setHeight:(CGFloat)height{
    _height = height;
    [[NSNotificationCenter defaultCenter] postNotificationName:HomeTableHeadViewHeightDidChange object:[NSNumber numberWithFloat:_height]];
}

- (void)layoutSubviews{
    [super layoutSubviews];
    self.height = CGRectGetMaxY(_scrollerView.frame);
}
