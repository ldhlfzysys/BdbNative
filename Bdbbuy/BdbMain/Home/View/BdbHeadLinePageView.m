...
#import "BdbHeadlineContentView.h"

@interface BdbHeadLinePageView ()<UIScrollViewDelegate>
@property (nonatomic, strong) UIScrollView *headlineScrollView;
@property (nonatomic, assign) NSInteger currentPage;
@property (nonatomic, strong) NSTimer *timer;
@end
@implementation BdbHeadLinePageView
/* 最大重用view的个数 */
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i < MaxContentViewCount; ++i) {
            BdbHeadlineContentView *contentView = [[BdbHeadlineContentView alloc]init];
...
- (void)layoutSubviews{
    ...
    for (NSInteger i = 0; i<MaxContentViewCount; i++) {
        BdbHeadlineContentView *contentView = self.headlineScrollView.subviews[i];
