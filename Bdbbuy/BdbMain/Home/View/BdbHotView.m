...
- (instancetype)initWithImages:(NSArray<NSString *> *)images title:(NSArray *)titles placeHolder:(UIImage *)image {
    ...
    for (NSInteger i = 0; i<images.count; i++) {
        BdbIconImageTextView *iconView = [BdbIconImageTextView IconImageTextView:images[i] title:titles[i] placeHolder:image];
        iconView.tag = i;
...
- (void)layoutSubviews {
    ...
    for (NSInteger i = 0; i<self.subviews.count; i++) {
        BdbIconImageTextView *iconView = self.subviews[i];
        NSLog(@"iconView = %@",iconView);
