...
+(BaseTabBarViewController *)sharedController{
    @synchronized(self){
        if(controller == nil){
            ...
            NSArray *titles = @[@"首页",@"挑选商品",@"购物车",@"我的"];
            NSArray *vcs = @[[[BdbHomeViewController alloc]init],[[BdbFlashViewController alloc]init],[[BdbShoppingVC alloc]init],[[BdbMyViewController alloc]init]];
            NSArray *images = @[@"v2_home",@"v2_order",@"shopCart",@"v2_my"];
            NSArray *selectedImages = @[@"v2_home_r",@"v2_order_r",@"shopCart_r",@"v2_my_r"];
            [controller addChildVc:vcs titles:titles images:images selectedImages:selectedImages tabBarNaviChildVC:[[BdbBaseNavigationController alloc]init]];
        }
    }
    return controller;
}
...
- (void)addChildVc:(NSArray *)childVcs titles:(NSArray *)titles images:(NSArray *)images selectedImages:(NSArray *)selectedImages tabBarNaviChildVC:(BdbBaseNavigationController *)tabBarNaviChildVC{
